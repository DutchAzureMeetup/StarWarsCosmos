...
namespace CosmosDBChangeFeed
{
    class Program
    {
        ...
        public async Task RunChangeFeedHostAsync()
        {
            ...
            using (DocumentClient destClient = new DocumentClient(documentCollectionLocation.Uri, documentCollectionLocation.MasterKey))
            {
                DocumentFeedObserverFactory docObserverFactory = new DocumentFeedObserverFactory(destClient, documentCollectionLocation);
